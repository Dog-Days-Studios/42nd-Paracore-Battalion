...
private _ropeLength = _lineStartPosASL distance _topPosASL;
if (_ropeLength > 0.25) then {
    _persistentRope = ropeCreate [_anchor, [0, 0, 0], _startHelper, [0, 0, 0], _ropeLength];

    // Add a visible physical rope object to replace default thin white rope visuals
    _ropeVisual = createVehicle ["Land_Rope_F", _lineStartPosASL, [], 0, "CAN_COLLIDE"];
    _ropeVisual setPosASL _lineStartPosASL;
    _ropeVisual setVectorDirAndUp [_topPosASL vectorDiff _lineStartPosASL, [0,0,1]];
