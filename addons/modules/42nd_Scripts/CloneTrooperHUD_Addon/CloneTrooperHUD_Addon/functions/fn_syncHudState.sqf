...
private _isFirstPerson = (toUpper cameraView) isEqualTo "INTERNAL";
...
if (_shouldShowHud) then
{
    ...
    if (isNull _display || {_activeDisplayClass != _displayClass}) then
    {
        cutRsc [_displayClass, "PLAIN"];
        missionNamespace setVariable ["CTHUD_activeDisplayClass", _displayClass];
        _display = uiNamespace getVariable ["CTHUD_Display", displayNull];
    }
    else
    {
