/*
    ...
    [this] execVM "\42nd_para\42nd\addons\modules\42nd_Scripts\resupply\gozanti_spawner.sqf";
*/
...
if (isNil "para42_fnc_gozantiLoadPresetCargo") then {
    para42_fnc_gozantiLoadPresetCargo = {
        params [
            ["_transport", objNull, [objNull]],
        ...
        if (!isServer) exitWith {
            [_transport, _caller, _cargoClass, _cargoLabel] remoteExecCall ["para42_fnc_gozantiLoadPresetCargo", 2];
        };
        ...
        private _spawnPos = _transport modelToWorld [0, -4, -8];
        private _cargo = createVehicle [_cargoClass, _spawnPos, [], 0, "CAN_COLLIDE"];
        if (isNull _cargo) exitWith {
            [_caller, format ["Unable to spawn %1.", _cargoLabel]] call para42_fnc_gozantiNotify;
        };
        ...
        if ([_transport, _cargo] call para42_fnc_gozantiLoadCargo) then {
            private _cargoName = _cargoLabel;
            if (_cargoName isEqualTo "") then {
                _cargoName = getText (configOf _cargo >> "displayName");
            };
            if (_cargoName isEqualTo "") then {
            ...
            [_caller, format ["Unable to load %1.", _cargoLabel]] call para42_fnc_gozantiNotify;
        };
    };
};
...
if (isNil "para42_fnc_gozantiDropVehicle") then {
    para42_fnc_gozantiDropVehicle = {
        ...
        if (_loadedVehicles isEqualTo []) exitWith {
            [_caller, "There is no loaded cargo to drop."] call para42_fnc_gozantiNotify;
        };
        ...
        if ([_transport, _cargo] call para42_fnc_gozantiDropCargo) then {
            [_caller, format ["Cargo dropped (%1 remaining).", count _loadedVehicles]] call para42_fnc_gozantiNotify;
        };
    };
};
    ...
    para42_fnc_gozantiDropAllVehicles = {
        ...
        [_transport, _caller, _loadedVehicles] spawn {
            ...
            for "_i" from ((count _loadedVehicles) - 1) to 0 step -1 do {
                ...
                [_caller, format ["Airdropped %1/%2 cargo item(s).", (count _loadedVehicles) - _i, count _loadedVehicles]] call para42_fnc_gozantiNotify;
...

{
    ...
    _gozanti addAction [
        _actionTitle,
        {
            ...
            [_target, _caller, _cargoClass, _cargoLabel] remoteExecCall ["para42_fnc_gozantiLoadPresetCargo", 2];
        },
...
_gozanti addAction [
    "<t color='#9fd3ff'>Airdrop All Loaded Cargo</t>",
    {
        params ["_target", "_caller"];
